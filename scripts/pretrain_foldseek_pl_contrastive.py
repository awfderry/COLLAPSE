...
import numpy as np
from collapse.byol_pytorch import BYOL_Contrastive
from sklearn.neighbors import KNeighborsClassifier
# from sklearn.linear_model import LogisticRegression
from sklearn.model_selection import cross_val_score

from collapse.models import CDDModel
from collapse.data import FoldseekContrastiveDataset, NoneCollater
...
def dynamic_negative_sampling(anchor_embedding, negative_embeddings, K=5):
    # Compute cosine similarities between the anchor embedding and negative embeddings
    similarities = F.cosine_similarity(anchor_embedding.unsqueeze(0), negative_embeddings)
    
    # Get the indices of the top K most similar negatives
    top_k_indices = torch.topk(similarities, K).indices
    
    # Randomly select one from the top K
    selected_idx = random.choice(top_k_indices).item()
    
    return selected_idx


class CollapseModel(pl.LightningModule):
    def __init__(self, byol_model, train_dataset, **kwargs):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        #(graph_anc, graph_pos, graph_neg), _ = batch
        (graph_anc, graph_pos, _), _ = batch  # Ignore the pre-sampled negatives
    
        # Forward pass for anchor and positive samples to get embeddings
        anchor_embeddings = self.byol_model.encode(graph_anc)
        positive_embeddings = self.byol_model.encode(graph_pos)

        # Sample a shared pool of random positions for the entire batch
        # ATTENTION!: does this get the number of anchors correctly
        num_anchors = graph_anc.size(0)
        # TODO!: implement the sampling
        random_positions = self.train_dataset.getNegatives(num_anchors)  # Sample a set of random positions equal to the number of anchors
        random_embeddings = self.byol_model.encode(random_positions) # precompute their embeddings with the latest state of the model

        # Dynamically sample negatives for each anchor from the shared pool
        negative_indices = [dynamic_negative_sampling(anchor, random_embeddings) for anchor in anchor_embeddings]
        negative_embeddings = torch.stack([negative_embeddings[idx] for idx in negative_indices])

        # Compute the loss using anchor, positive, and dynamically sampled negatives
        loss = self.loss(anchor_embeddings, positive_embeddings, negative_embeddings)
        #loss = self.byol_model.compute_loss(anchor_embeddings, positive_embeddings, selected_negatives)
        
        #loss = self.byol_model(graph_anc, graph_pos, graph_neg)
        self.logger.experiment.log({'loss': loss.item()})
        return loss
    
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        # TODO!: change this to make it very similar to the training_steo.
        if dataloader_idx == 0:
            # Process batch from val_loader
            (graph_anc, graph_pos, graph_neg), meta = batch
            res1, res2, res3 = meta['res_labels']
            loss = self.byol_model(graph_anc, graph_pos, graph_neg)
            a, b, c = self.byol_model(graph_anc, return_embedding=True)
            pos_cosine = F.cosine_similarity(a, b)
            neg_cosine = F.cosine_similarity(a, c)
            cls_x = torch.cat([a, b, c])
            cls_y = torch.cat([res1, res2, res3])
            
            return {'loss': loss, 'cls_x': cls_x, 'cls_y': cls_y, 'pos_cosine': pos_cosine, 'neg_cosine': neg_cosine}
            
        elif dataloader_idx == 1:
            # Process batch from prosite_loader
            g, label = batch
...
def main(args):
    ...
    train_dataset = FoldseekContrastiveDataset(args.train_dataset, '/scratch/users/aderry/foldseek/pdb.lookup', '/scratch/users/aderry/pdb_lmdb', num_positions=1)
    val_dataset = FoldseekContrastiveDataset(args.val_dataset, '/scratch/users/aderry/foldseek/pdb.lookup', '/scratch/users/aderry/pdb_lmdb', num_positions=1)
    
    dummy_graph = torch.load(os.path.join(os.environ["DATA_DIR"], 'dummy_graph.pt'))
    
    byol_model = BYOL_Contrastive(
